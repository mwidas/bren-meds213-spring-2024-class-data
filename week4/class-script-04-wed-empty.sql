...
-- Ask 1.2: What is the average snow cover at each site for each year?
SELECT Site, Year, AVG(Snow_cover) FROM Snow_cover
    GROUP BY Site, Year;   

-- Ask 2: Order the result to get the top 3 snowy sites?
SELECT Site, AVG(Snow_cover) AS Snow_cover_Avg FROM Snow_cover
    GROUP BY Site
    ORDER BY Snow_cover_Avg DESC
    LIMIT 3;
...
DROP VIEW Site_avg_snowcover_nozeros;
...
-- need to join first to do this
SELECT Site, Site_avg_snowcover_nozeros.Snow_cover_Avg - Site_avg_snowcover.Snow_cover_Avg AS avg_diff
...
SELECT * FROM Site_avg_snowcover;
-- What just happened!?
-- we saved it as a temporary table so this did not update
